...
from plot_lib.library.utilities.processes import get_manager_processes
...
def get_job_data(jobs, running_work, as_json=False):
    rows = []
    added_pids = []
    for job in jobs:
        for pid in job.running_work:
            if pid not in running_work:
                continue
            rows.append(_get_row_info(pid, running_work, as_json))
